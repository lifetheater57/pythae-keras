...
@pytest.fixture(params=[CIWAEConfig(), CIWAEConfig(latent_dim=5, beta=1)])
...
class Test_Model_Building:
    ...
    def test_raises_bad_inheritance(self, model_configs, bad_net):
        with pytest.raises(BadInheritanceError):
            model = CIWAE(model_configs, encoder=bad_net)

        with pytest.raises(BadInheritanceError):
            model = CIWAE(model_configs, decoder=bad_net)

    def test_raises_no_input_dim(
        self, model_configs_no_input_dim, custom_encoder, custom_decoder
    ):
        with pytest.raises(AttributeError):
            model = CIWAE(model_configs_no_input_dim)

        with pytest.raises(AttributeError):
            model = CIWAE(model_configs_no_input_dim, encoder=custom_encoder)

        with pytest.raises(AttributeError):
            model = CIWAE(model_configs_no_input_dim, decoder=custom_decoder)
    ...
    def test_build_custom_arch(self, model_configs, custom_encoder, custom_decoder):
        model = CIWAE(model_configs, encoder=custom_encoder, decoder=custom_decoder)
        ...
        model = CIWAE(model_configs, encoder=custom_encoder)

        assert model.encoder == custom_encoder
        assert not model.model_config.uses_default_encoder
        assert model.model_config.uses_default_decoder

        model = CIWAE(model_configs, decoder=custom_decoder)
...
class Test_Model_forward:
    ...
    @pytest.fixture
    def CIWAE(self, model_configs, demo_data):
        model_configs.input_dim = tuple(demo_data["data"][0].shape)
        return CIWAE(model_configs)

    def test_model_train_output(self, CIWAE, demo_data):
        CIWAE.train()
...
class Test_NLL_Compute:
    ...
    def test_nll_compute(self, CIWAE, demo_data, nll_params):
        nll = CIWAE.get_nll(
...
class Test_CIWAE_Training:
    ...
    def test_checkpoint_saving(self, ciwae, trainer, training_configs):
        ...
        model_rec = CIWAE.load_from_folder(os.path.join(checkpoint_dir))
    ...
    def test_checkpoint_saving_during_training(self, ciwae, trainer, training_configs):
        ...
        training_dir = os.path.join(
            dir_path, f"CIWAE_training_{trainer._training_signature}"
        )
    ...
    def test_CIWAE_training_pipeline(self, ciwae, train_dataset, training_configs):
        ...
        training_dir = os.path.join(
            dir_path, f"CIWAE_training_{pipeline.trainer._training_signature}"
        )
        assert os.path.isdir(training_dir)
...
class Test_CIWAE_Generation:
    ...
    @pytest.fixture()
    def ae_model(self):
        return CIWAE(CIWAEConfig(input_dim=(1, 28, 28), latent_dim=7))
