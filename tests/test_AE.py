...
@pytest.fixture(params=[AEConfig(), AEConfig(latent_dim=5)])
...
@pytest.fixture(
    params=[
        ...
        AEConfig(input_dim=(1, 2, 18), latent_dim=5),
    ]
)
...
class Test_Model_Building:
    ...
    def test_raises_bad_inheritance(self, model_configs, bad_net):
        with pytest.raises(BadInheritanceError):
            model = AE(model_configs, encoder=bad_net)

        with pytest.raises(BadInheritanceError):
            model = AE(model_configs, decoder=bad_net)

    def test_raises_no_input_dim(
        self, model_configs_no_input_dim, custom_encoder, custom_decoder
    ):
        with pytest.raises(AttributeError):
            model = AE(model_configs_no_input_dim)

        with pytest.raises(AttributeError):
            model = AE(model_configs_no_input_dim, encoder=custom_encoder)

        with pytest.raises(AttributeError):
            model = AE(model_configs_no_input_dim, decoder=custom_decoder)
...
class Test_Model_forward:
    ...
    def test_model_train_output(self, ae, demo_data):
        ...
        assert set(["loss", "recon_x", "z"]) == set(out.keys())
...
class Test_AE_Training:
    ...
    def ae(self, model_configs, custom_encoder, custom_decoder, request):
        # randomized
    ...
    def test_checkpoint_saving(self, ae, trainer, training_configs):
        ...
        if not ae.model_config.uses_default_decoder:
        ...
        if not ae.model_config.uses_default_encoder:
    ...
    def test_checkpoint_saving_during_training(self, ae, trainer, training_configs):
        #
        ...
        training_dir = os.path.join(
            dir_path, f"AE_training_{trainer._training_signature}"
        )
    ...
    def test_ae_training_pipeline(self, tmpdir, ae, train_dataset, training_configs):
        ...
        training_dir = os.path.join(
            dir_path, f"AE_training_{pipeline.trainer._training_signature}"
        )
        assert os.path.isdir(training_dir)
...
class Test_AE_Generation:
    ...
    @pytest.fixture()
    def ae_model(self):
        return AE(AEConfig(input_dim=(1, 28, 28), latent_dim=7))
