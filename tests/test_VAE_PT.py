...
class Test_Model_Building:
    ...
    def test_raises_bad_inheritance(self, model_configs, bad_net):
        with pytest.raises(BadInheritanceError):
            model = VAE_PT(model_configs, encoder=bad_net)

        with pytest.raises(BadInheritanceError):
            model = VAE_PT(model_configs, decoder=bad_net)

    def test_raises_no_input_dim(
        self, model_configs_no_input_dim, custom_encoder, custom_decoder
    ):
        with pytest.raises(AttributeError):
            model = VAE_PT(model_configs_no_input_dim)

        with pytest.raises(AttributeError):
            model = VAE_PT(model_configs_no_input_dim, encoder=custom_encoder)

        with pytest.raises(AttributeError):
            model = VAE_PT(model_configs_no_input_dim, decoder=custom_decoder)
