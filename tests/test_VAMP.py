...
@pytest.fixture(params=[VAMPConfig(), VAMPConfig(latent_dim=5)])
...
class Test_Model_Building:
    ...
    def test_raises_bad_inheritance(self, model_configs, bad_net):
        with pytest.raises(BadInheritanceError):
            model = VAMP(model_configs, encoder=bad_net)

        with pytest.raises(BadInheritanceError):
            model = VAMP(model_configs, decoder=bad_net)

    def test_raises_no_input_dim(
        self, model_configs_no_input_dim, custom_encoder, custom_decoder
    ):
        with pytest.raises(AttributeError):
            model = VAMP(model_configs_no_input_dim)

        with pytest.raises(AttributeError):
            model = VAMP(model_configs_no_input_dim, encoder=custom_encoder)

        with pytest.raises(AttributeError):
            model = VAMP(model_configs_no_input_dim, decoder=custom_decoder)
...
class Test_NLL_Compute:
    ...
    def test_nll_compute(self, vamp, demo_data, nll_params):
        nll = vamp.get_nll(
...
class Test_VAMP_Training:
    ...
    @pytest.fixture
    def trainer(self, vamp, train_dataset, training_configs):
        trainer = BaseTrainer(
            model=vamp,
    ...
    def test_vamp_train_step(self, trainer):
    ...
    def test_checkpoint_saving(self, vamp, trainer, training_configs):
        ...
        if not vamp.model_config.uses_default_decoder:
        ...
        if not vamp.model_config.uses_default_encoder:
