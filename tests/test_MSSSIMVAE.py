...
from pythae.models import MSSSIM_VAE, AutoModel, MSSSIM_VAEConfig
...
@pytest.fixture(
    params=[MSSSIM_VAEConfig(), MSSSIM_VAEConfig(latent_dim=5, beta=5.0, window_size=5)]
)
...
class Test_Model_Building:
    ...
    def test_build_model(self, model_configs):
        model = MSSSIM_VAE(model_configs)
        assert all(
            [
    ...
    def test_raises_bad_inheritance(self, model_configs, bad_net):
        with pytest.raises(BadInheritanceError):
            model = MSSSIM_VAE(model_configs, encoder=bad_net)

        with pytest.raises(BadInheritanceError):
            model = MSSSIM_VAE(model_configs, decoder=bad_net)

    def test_raises_no_input_dim(
        self, model_configs_no_input_dim, custom_encoder, custom_decoder
    ):
        with pytest.raises(AttributeError):
            model = MSSSIM_VAE(model_configs_no_input_dim)

        with pytest.raises(AttributeError):
            model = MSSSIM_VAE(model_configs_no_input_dim, encoder=custom_encoder)

        with pytest.raises(AttributeError):
            model = MSSSIM_VAE(model_configs_no_input_dim, decoder=custom_decoder)
    ...
    def test_build_custom_arch(self, model_configs, custom_encoder, custom_decoder):
        ...
        model = MSSSIM_VAE(model_configs, encoder=custom_encoder)

        assert model.encoder == custom_encoder
        assert not model.model_config.uses_default_encoder
        assert model.model_config.uses_default_decoder

        model = MSSSIM_VAE(model_configs, decoder=custom_decoder)
...
class Test_Model_forward:
    ...
    @pytest.fixture
    def msssim_vae(self, model_configs, demo_data):
        model_configs.input_dim = tuple(demo_data["data"][0].shape)
        return MSSSIM_VAE(model_configs)

    def test_model_train_output(self, msssim_vae, demo_data):

        msssim_vae.train()

        out = msssim_vae(demo_data)
...
class Test_NLL_Compute:
    ...
    def test_nll_compute(self, msssim_vae, demo_data, nll_params):
        nll = msssim_vae.get_nll(
...
@pytest.mark.slow
class Test_MSSSIM_VAE_Training:
    @pytest.fixture
    def train_dataset(self):
        return torch.load(os.path.join(PATH, "data/mnist_clean_train_dataset_sample"))

    @pytest.fixture(
        params=[BaseTrainerConfig(num_epochs=3, steps_saving=2, learning_rate=1e-6)]
    )
    ...
    def msssim_vae(self, model_configs, custom_encoder, custom_decoder, request):
        # randomized
    ...
    @pytest.fixture
    def trainer(self, msssim_vae, train_dataset, training_configs):
        trainer = BaseTrainer(
            model=msssim_vae,
    ...
    def test_msssim_vae_train_step(self, trainer):
    ...
    def test_msssim_vae_eval_step(self, trainer):
    ...
    def test_msssim_vae_predict_step(self, trainer, train_dataset):
    ...
    def test_msssim_vae_main_train_loop(self, trainer):
    ...
    def test_checkpoint_saving(self, msssim_vae, trainer, training_configs):
        ...
        if not msssim_vae.model_config.uses_default_decoder:
        ...
        if not msssim_vae.model_config.uses_default_encoder:
        ...
        model_rec = msssim_vae.load_from_folder(os.path.join(checkpoint_dir))
    ...
    def test_checkpoint_saving_during_training(
        ...
        training_dir = os.path.join(
            dir_path, f"MSSSIM_VAE_training_{trainer._training_signature}"
        )
    ...
    def test_final_model_saving(self, msssim_vae, trainer, training_configs):
    ...
    def test_msssim_vae_training_pipeline(
        ...
        # build pipeline
        pipeline = TrainingPipeline(model=msssim_vae, training_config=training_configs)
        ...
        training_dir = os.path.join(
            dir_path, f"MSSSIM_VAE_training_{pipeline.trainer._training_signature}"
        )
        assert os.path.isdir(training_dir)
...
class Test_MSSSIM_VAE_Generation:
    @pytest.fixture
    def train_data(self):
        return torch.load(
    ...
    @pytest.fixture()
    def ae_model(self):
        return MSSSIM_VAE(MSSSIM_VAEConfig(input_dim=(1, 28, 28), latent_dim=7))
