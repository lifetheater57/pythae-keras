...
@pytest.fixture(params=[SVAEConfig(), SVAEConfig(latent_dim=5)])
...
class Test_Model_Building:
    ...
    def test_raises_bad_inheritance(self, model_configs, bad_net):
        with pytest.raises(BadInheritanceError):
            model = SVAE(model_configs, encoder=bad_net)

        with pytest.raises(BadInheritanceError):
            model = SVAE(model_configs, decoder=bad_net)

    def test_raises_no_input_dim(
        self, model_configs_no_input_dim, custom_encoder, custom_decoder
    ):
        with pytest.raises(AttributeError):
            model = SVAE(model_configs_no_input_dim)

        with pytest.raises(AttributeError):
            model = SVAE(model_configs_no_input_dim, encoder=custom_encoder)

        with pytest.raises(AttributeError):
            model = SVAE(model_configs_no_input_dim, decoder=custom_decoder)
    ...
    def test_build_custom_arch(self, model_configs, custom_encoder, custom_decoder):
        ...
        model = SVAE(model_configs, encoder=custom_encoder)
...
class Test_Model_forward:
    ...
    def test_model_train_output(self, svae, demo_data):
        svae.train()
...
class Test_NLL_Compute:
    ...
    def test_nll_compute(self, svae, demo_data, nll_params):
        nll = svae.get_nll(
...
class Test_SVAE_Training:
    ...
    def svae(self, model_configs, custom_encoder, custom_decoder, request):
        # randomized
    ...
    def test_checkpoint_saving(self, svae, trainer, training_configs):
        ...
        if not svae.model_config.uses_default_decoder:
        ...
        if not svae.model_config.uses_default_encoder:
        ...
        model_rec = svae.load_from_folder(os.path.join(checkpoint_dir))
    ...
    def test_checkpoint_saving_during_training(self, svae, trainer, training_configs):
        ...
        training_dir = os.path.join(
            dir_path, f"SVAE_training_{trainer._training_signature}"
        )
    ...
    def test_svae_training_pipeline(self, svae, train_dataset, training_configs):
        ...
        training_dir = os.path.join(
            dir_path, f"SVAE_training_{pipeline.trainer._training_signature}"
        )
        assert os.path.isdir(training_dir)
