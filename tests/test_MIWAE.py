...
@pytest.fixture(
    params=[MIWAEConfig(), MIWAEConfig(latent_dim=5, number_gradient_estimates=3)]
)
...
class Test_Model_Building:
    ...
    def test_raises_bad_inheritance(self, model_configs, bad_net):
        with pytest.raises(BadInheritanceError):
            model = MIWAE(model_configs, encoder=bad_net)

        with pytest.raises(BadInheritanceError):
            model = MIWAE(model_configs, decoder=bad_net)

    def test_raises_no_input_dim(
        self, model_configs_no_input_dim, custom_encoder, custom_decoder
    ):
        with pytest.raises(AttributeError):
            model = MIWAE(model_configs_no_input_dim)

        with pytest.raises(AttributeError):
            model = MIWAE(model_configs_no_input_dim, encoder=custom_encoder)

        with pytest.raises(AttributeError):
            model = MIWAE(model_configs_no_input_dim, decoder=custom_decoder)
    ...
    def test_build_custom_arch(self, model_configs, custom_encoder, custom_decoder):
        model = MIWAE(model_configs, encoder=custom_encoder, decoder=custom_decoder)
        ...
        model = MIWAE(model_configs, encoder=custom_encoder)

        assert model.encoder == custom_encoder
        assert not model.model_config.uses_default_encoder
        assert model.model_config.uses_default_decoder

        model = MIWAE(model_configs, decoder=custom_decoder)
...
class Test_Model_forward:
    ...
    @pytest.fixture
    def MIWAE(self, model_configs, demo_data):
        model_configs.input_dim = tuple(demo_data["data"][0].shape)
        return MIWAE(model_configs)

    def test_model_train_output(self, MIWAE, demo_data):
        MIWAE.train()
...
class Test_NLL_Compute:
    ...
    def test_nll_compute(self, MIWAE, demo_data, nll_params):
        nll = MIWAE.get_nll(
...
class Test_MIWAE_Training:
    ...
    def test_miwae_predict_step(self, trainer, train_dataset):
        start_model_state_dict = deepcopy(trainer.model.state_dict())
    ...
    def test_miwae_main_train_loop(self, trainer):
        start_model_state_dict = deepcopy(trainer.model.state_dict())
    ...
    def test_checkpoint_saving(self, miwae, trainer, training_configs):
        ...
        model_rec = MIWAE.load_from_folder(os.path.join(checkpoint_dir))
    ...
    def test_checkpoint_saving_during_training(self, miwae, trainer, training_configs):
        #
        ...
        training_dir = os.path.join(
            dir_path, f"MIWAE_training_{trainer._training_signature}"
        )
    ...
    def test_final_model_saving(self, miwae, trainer, training_configs):
        dir_path = training_configs.output_dir
    ...
    def test_MIWAE_training_pipeline(self, miwae, train_dataset, training_configs):
        dir_path = training_configs.output_dir
        ...
        training_dir = os.path.join(
            dir_path, f"MIWAE_training_{pipeline.trainer._training_signature}"
        )
        assert os.path.isdir(training_dir)
...
class Test_MIWAE_Generation:
    ...
    def ae_model(self):
        return MIWAE(
            MIWAEConfig(
                input_dim=(1, 28, 28), latent_dim=7, number_gradient_estimates=2
            )
        )
