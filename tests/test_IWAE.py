...
class Test_Model_Building:
    ...
    def test_raises_bad_inheritance(self, model_configs, bad_net):
        with pytest.raises(BadInheritanceError):
            model = IWAE(model_configs, encoder=bad_net)

        with pytest.raises(BadInheritanceError):
            model = IWAE(model_configs, decoder=bad_net)

    def test_raises_no_input_dim(
        self, model_configs_no_input_dim, custom_encoder, custom_decoder
    ):
        with pytest.raises(AttributeError):
            model = IWAE(model_configs_no_input_dim)

        with pytest.raises(AttributeError):
            model = IWAE(model_configs_no_input_dim, encoder=custom_encoder)

        with pytest.raises(AttributeError):
            model = IWAE(model_configs_no_input_dim, decoder=custom_decoder)
    ...
    def test_build_custom_arch(self, model_configs, custom_encoder, custom_decoder):
        ...
        model = IWAE(model_configs, encoder=custom_encoder)
...
class Test_NLL_Compute:
    ...
    def test_nll_compute(self, iwae, demo_data, nll_params):
        nll = iwae.get_nll(
...
class Test_IWAE_Training:
    ...
    def iwae(self, model_configs, custom_encoder, custom_decoder, request):
        # randomized
    ...
    def test_checkpoint_saving(self, iwae, trainer, training_configs):
        ...
        if not iwae.model_config.uses_default_decoder:
        ...
        if not iwae.model_config.uses_default_encoder:
        ...
        model_rec = iwae.load_from_folder(os.path.join(checkpoint_dir))
    ...
    def test_checkpoint_saving_during_training(self, iwae, trainer, training_configs):
        #
        ...
        training_dir = os.path.join(
            dir_path, f"IWAE_training_{trainer._training_signature}"
        )
    ...
    def test_iwae_training_pipeline(
        ...
        training_dir = os.path.join(
            dir_path, f"IWAE_training_{pipeline.trainer._training_signature}"
        )
        assert os.path.isdir(training_dir)
