...
@pytest.fixture(params=[HVAEConfig(), HVAEConfig(latent_dim=5)])
...
class Test_Model_Building:
    ...
    def test_raises_bad_inheritance(self, model_configs, bad_net):
        with pytest.raises(BadInheritanceError):
            model = HVAE(model_configs, encoder=bad_net)

        with pytest.raises(BadInheritanceError):
            model = HVAE(model_configs, decoder=bad_net)

    def test_raises_no_input_dim(
        self, model_configs_no_input_dim, custom_encoder, custom_decoder
    ):
        with pytest.raises(AttributeError):
            model = HVAE(model_configs_no_input_dim)

        with pytest.raises(AttributeError):
            model = HVAE(model_configs_no_input_dim, encoder=custom_encoder)

        with pytest.raises(AttributeError):
            model = HVAE(model_configs_no_input_dim, decoder=custom_decoder)
...
class Test_NLL_Compute:
    ...
    def test_nll_compute(self, hvae, demo_data, nll_params):
        nll = hvae.get_nll(
...
class Test_HVAE_Generation:
    ...
    @pytest.fixture()
    def ae_model(self):
        return HVAE(HVAEConfig(input_dim=(1, 28, 28), latent_dim=7))
