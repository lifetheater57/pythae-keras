...
@pytest.fixture(params=[RAE_GP_Config(), RAE_GP_Config(latent_dim=5)])
...
@pytest.fixture(
    params=[
        RAE_GP_Config(input_dim=(1, 28, 28), latent_dim=10, reg_weight=1e-2),
        RAE_GP_Config(
            input_dim=(1, 2, 18), latent_dim=5, reg_weight=1.0, embedding_weight=0.2
        ),
    ]
)
...
class Test_Model_Building:
    ...
    def test_raises_bad_inheritance(self, model_configs, bad_net):
        with pytest.raises(BadInheritanceError):
            model = RAE_GP(model_configs, encoder=bad_net)

        with pytest.raises(BadInheritanceError):
            model = RAE_GP(model_configs, decoder=bad_net)

    def test_raises_no_input_dim(
        self, model_configs_no_input_dim, custom_encoder, custom_decoder
    ):
        with pytest.raises(AttributeError):
            model = RAE_GP(model_configs_no_input_dim)

        with pytest.raises(AttributeError):
            model = RAE_GP(model_configs_no_input_dim, encoder=custom_encoder)

        with pytest.raises(AttributeError):
            model = RAE_GP(model_configs_no_input_dim, decoder=custom_decoder)
    ...
    def test_build_custom_arch(self, model_configs, custom_encoder, custom_decoder):
        model = RAE_GP(model_configs, encoder=custom_encoder, decoder=custom_decoder)
        ...
        model = RAE_GP(model_configs, encoder=custom_encoder)

        assert model.encoder == custom_encoder
        assert not model.model_config.uses_default_encoder
        assert model.model_config.uses_default_decoder

        model = RAE_GP(model_configs, decoder=custom_decoder)
...
class Test_RAE_GP_Training:
    ...
    def test_checkpoint_saving_during_training(self, rae, trainer, training_configs):
        ...
        training_dir = os.path.join(
            dir_path, f"RAE_GP_training_{trainer._training_signature}"
        )
    ...
    def test_rae_training_pipeline(self, rae, train_dataset, training_configs):
        ...
        training_dir = os.path.join(
            dir_path, f"RAE_GP_training_{pipeline.trainer._training_signature}"
        )
        assert os.path.isdir(training_dir)
...
class Test_RAE_Generation:
    ...
    @pytest.fixture()
    def ae_model(self):
        return RAE_GP(RAE_GP_Config(input_dim=(1, 28, 28), latent_dim=7))
