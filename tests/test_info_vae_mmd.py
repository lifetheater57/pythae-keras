...
@pytest.fixture(params=[INFOVAE_MMD_Config(), INFOVAE_MMD_Config(latent_dim=5)])
...
class Test_Model_Building:
    ...
    def test_build_model(self, model_configs):
        model = INFOVAE_MMD(model_configs)
        assert all(
            [
    ...
    def test_raises_bad_inheritance(self, model_configs, bad_net):
        with pytest.raises(BadInheritanceError):
            model = INFOVAE_MMD(model_configs, encoder=bad_net)

        with pytest.raises(BadInheritanceError):
            model = INFOVAE_MMD(model_configs, decoder=bad_net)

    def test_raises_no_input_dim(
        self, model_configs_no_input_dim, custom_encoder, custom_decoder
    ):
        with pytest.raises(AttributeError):
            model = INFOVAE_MMD(model_configs_no_input_dim)

        with pytest.raises(AttributeError):
            model = INFOVAE_MMD(model_configs_no_input_dim, encoder=custom_encoder)

        with pytest.raises(AttributeError):
            model = INFOVAE_MMD(model_configs_no_input_dim, decoder=custom_decoder)
    ...
    def test_build_custom_arch(self, model_configs, custom_encoder, custom_decoder):
        ...
        model = INFOVAE_MMD(model_configs, encoder=custom_encoder)

        assert model.encoder == custom_encoder
        assert not model.model_config.uses_default_encoder
        assert model.model_config.uses_default_decoder

        model = INFOVAE_MMD(model_configs, decoder=custom_decoder)
...
class Test_Model_Saving:
    def test_default_model_saving(self, tmpdir, model_configs):
        ...
        # reload model
        model_rec = INFOVAE_MMD.load_from_folder(dir_path)
...
class Test_NLL_Compute:
    ...
    def test_nll_compute(self, info_vae_mmd, demo_data, nll_params):
        nll = info_vae_mmd.get_nll(
...
class Test_INFOVAE_MMD_Training:
    ...
    def info_vae_mmd(self, model_configs, custom_encoder, custom_decoder, request):
        # randomized
    ...
    def test_checkpoint_saving(self, info_vae_mmd, trainer, training_configs):
        ...
        if not info_vae_mmd.model_config.uses_default_decoder:
        ...
        if not info_vae_mmd.model_config.uses_default_encoder:
        ...
        model_rec = info_vae_mmd.load_from_folder(os.path.join(checkpoint_dir))
    ...
    def test_checkpoint_saving_during_training(
        ...
        training_dir = os.path.join(
            dir_path, f"INFOVAE_MMD_training_{trainer._training_signature}"
        )
    ...
    def test_final_model_saving(self, info_vae_mmd, trainer, training_configs):
        ...
        # check reload full model
        model_rec = INFOVAE_MMD.load_from_folder(os.path.join(final_dir))
    ...
    def test_info_vae_mmd_training_pipeline(
        ...
        training_dir = os.path.join(
            dir_path, f"INFOVAE_MMD_training_{pipeline.trainer._training_signature}"
        )
        assert os.path.isdir(training_dir)
